...
$(document).ready( function() {
    ...
    $.getScript("C:/Users/Mason/Dropbox/Projects/Web/Editor/Languages/sas.js", function() {
	...
	*****/
	...
	function Editor() {
		...
		self.languages = { // Language-specific properties for coloring, shorthand, indenting, etc.
            sas : {
                colorList : {
            ...
            r : {
                colorList : {
                    regexs : [/([a-zA-Z0-9_\.]+)(?=\()/ig,				            // functions
                          /("[^"]*")/ig,						                                 // strings
                          /((\s+|^)(break|else|if|next|while))/ig,	// control flow
                          /((^|\s+)-?\d+(.?\d+)?)(?=(\D+|$|;))/g,			                    // numbers, notices decimals
                          /(#.+)/ig,					                        // comments
                        ],
                    replace : ["$1", "$1", "$1", "$1", "$1"],
                    colors : ["function", "string", "control-flow", "number", "comment"]
                    },
        ...
        self.language = self.languages["r"];
		...
		self.lineWords = ko.computed( function() {
		        var text = self.text()[self.cLine()]()
		        var textWords = (text === "") ? [] : text.split(/[\s;\(\)"'\?\=]+/ig);
...
//console.log(key);
	...
	*****/
    ...
    /* New editor, and load language file. */
	editor = new Editor();	
